"""
過去に投稿済みの記事に対して、下記2つを後付け・自動修正する専用スクリプト。
main.py と同じディレクトリに置いて実行すること（main.py の資産をそのまま再利用する）。

  1. チーム別カテゴリタグ（2つ目の<category>）と関連記事バナーの後付け
     - 対象: <category>が1つしか付いていない記事（＝チームタグ未付与の記事）
     - タイトル・本文をAIに読ませてチーム名を再抽出する

  2. サムネイル（バナー）画像URLのズレ修正
     - main.py の THUMBNAIL_IMAGES 辞書を差し替えた後、過去記事の <img> タグは
       投稿時点の古いURLのまま残ってしまう問題に対応。
     - build_blog_body() が生成する <img src="{URL}" alt="{カテゴリコード}" class="article-thumbnail" ...>
       の alt 属性には内部カテゴリコード（例: "SOCCER"）がそのまま入っているため、
       それをキーに THUMBNAIL_IMAGES の「今の」URLと突き合わせ、ズレていればsrcだけ差し替える。
     - AI呼び出しは発生しないため無料枠を消費しない。何度でも安全に再実行できる
       （既に最新URLの記事は自動的にスキップされるだけなので、専用の完了ログは持たない）。
...
- サムネイル画像URLの修正は毎回「今のTHUMBNAIL_IMAGES辞書」と実際のimgタグを比較するだけなので、
  ズレがなければ自動的に何もしない。ログファイルへの記録は不要（画像を差し替えるたびに何度でも実行してよい）
"""
...
# main.py の既存資産（環境変数読み込み・カテゴリURL組み立て・サムネイル辞書・AI呼び出し）を再利用する
from main import (
    LIVEDOOR_BLOG_ID,
    LIVEDOOR_API_KEY,
    GEMINI_API_KEY,
...
def fix_thumbnail_src(body_html, new_src):
    def _replace(m):
        return f'<img src="{new_src}" alt="{m.group(2)}" class="article-thumbnail"'
...
def inject_related_banner(body_html, team_name):
    if "related-team-section" in body_html:
    ...
    updated_html = body_html.replace(marker, related_html + "\n        " + marker, 1)
    return updated_html, True


def update_entry(entry_elem, team_name=None, new_thumbnail_src=None):
    edit_href = None
    ...
    banner_added = False
    thumb_fixed = False
    if content_elm is not None and content_elm.text:
        body_html = content_elm.text

        if team_name:
            body_html, banner_added = inject_related_banner(body_html, team_name)

        if new_thumbnail_src:
            fixed_html = fix_thumbnail_src(body_html, new_thumbnail_src)
            thumb_fixed = fixed_html != body_html
            body_html = fixed_html

        if banner_added or thumb_fixed:
    ...
    if DRY_RUN:
        print(f"[DRY RUN] 更新をスキップ（実際には送信しません）: team={team_name} / banner_added={banner_added} / thumb_fixed={thumb_fixed} / edit_href={edit_href}")
        return True
    ...
    try:
        ...
        if response.status_code in (200, 204):
            print(f"更新に成功しました: team={team_name} / banner_added={banner_added} / thumb_fixed={thumb_fixed}")
            return True
...
def main():
    ...
    for entry_elem in entries:
        ...
        current_thumb_src, category_code = get_thumbnail_info(body_html)
        expected_thumb_src = THUMBNAIL_IMAGES.get(category_code) if category_code else None
        needs_thumb_fix = bool(
            expected_thumb_src and current_thumb_src and expected_thumb_src != current_thumb_src
        )

        if not needs_team and not needs_thumb_fix:
            continue
        ...
        if needs_thumb_fix:
            print(f"[{processed_count}] バナー画像URLのズレを検出: {title}")
            print(f"  旧: {current_thumb_src}")
            print(f"  新: {expected_thumb_src}")

        success = update_entry(
            entry_elem,
            team_name=team_name,
            new_thumbnail_src=expected_thumb_src if needs_thumb_fix else None,
        )
