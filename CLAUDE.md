...
2. **【要検証】クリケット等の重複投稿修正（4-22参照）の本番効果確認**：デコードリトライ＋正規化URL比較を入れた後、実際に同一記事の二重投稿が再発していないか数日分の投稿で確認
3. **【要検証】記事本文の厚み増強（4-22参照）の本番効果確認**：【背景・経緯】行が実際に期待通りの内容（過去の類似事例との比較等）で生成されているか、文字数バランスは崩れていないかを確認
4. **【要検証】海外ソース拡張（4-20参照）の本番効果確認**：ESPN RSS・英語版Google News検索から実際に投稿が生成されるか、出典表示が正しく海外メディア名になるかを次回以降の投稿で確認
5. **【要検証】カテゴリ均等抽選（4-20参照）の本番効果確認**：数日〜1週間程度の投稿を見て、サッカー以外のカテゴリの投稿頻度が実際に改善しているか確認
6. **【要検証】タイトル/本文プロンプト改善（4-20参照）の本番効果確認**：新しいタイトル・SUMMARY構成で生成された記事の品質・文字数バランスを確認
7. **本番投稿の継続監視**：投稿頻度、カテゴリの偏り、記事品質などを定点観測
8. （優先度低）バリューコマース（U-NEXT）の審査結果確認。通過していれば該当カテゴリにリンクを追加登録してもよい（ABEMAとのA/Bテスト構成にできる）
9. （優先度低）DAZN系ASPの再挑戦：4-14で運営者情報等ページを設置済みなので、時間を空けて再申請する余地はあるが、4-16の通り現状はABEMA運用で確定・急ぎではない
10. **【要検証】SEO Short-term施策（4-19参照）の本番効果確認**：meta description空白バグの実際の解消具合、NewsArticle構造化データのGoogle認識状況をSearch Consoleで確認
